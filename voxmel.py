...
def extract_melody(
    ...
    **kwargs,
    ...
    audio : array_like, optional
        ...
        if `fp` is not provided. Default None.
    ...
    pitch_method : {'praat', 'crepe'}, optional
        Algorithm to extract the pitch contour. Default 'praat'.
    ...
    **kwargs : optional
    ...
    if sf not in {32000, 44100, 48000}:
        raise ValueError(
            "Sample frequency `sf` {} not in {32000, 44100, 48000}.".format(sf)
        )
    ...
    nuclei = segment_notes_peaks(I, p, ts, verbose=verbose, **kwargs)
...
def segment_notes(
    ...
    **kwargs,
    ...
    if verbose:
        ...
        print(
            f"intensity.shape={intensity.shape}, pitch.shape={pitch.shape}, ts.shape={ts.shape}, \
    ...
    for i in range(len(peaks_init)):
        ...
        margin = 0.1
        pitch_around = pitch[
            np.where(np.logical_and(ts >= p_i - margin, ts < p_i + margin))
        ]
        ...
        if I_p < threshold:
            ...
            remove.append(i)
            if verbose:
                print(" - peak at {:.4f} too low".format(p_i))
            ...
            if np.abs(intensity[peak_idx] - np.min(i_range)) < maxDip:
                ...
                remove.append(i)
                if verbose:
                    print(" - peak at {:.4f} too small".format(p_i))
        else:
            remove.append(i)
            if verbose:
                print(" - peak at {:.4f} unpitched".format(p_i))
    ...
    for i in range(0, len(peaks)):
        if i == 0:
            ...
            I_before = np.min(
                intensity[np.where(np.logical_and(ts >= peaks[i - 1], ts < peaks[i]))]
            )

        if i < len(peaks) - 1:
            I_after = np.min(
                intensity[np.where(np.logical_and(ts >= peaks[i], ts < peaks[i + 1]))]
            )
        else:
            I_after = 0
        ...
        if a and b:
            ...
            if verbose:
                print(" - peak at {:.4f} too small".format(peaks[i]))
...
def segment_notes_peaks(
    ...
    **kwargs,
    ...
    """Utilises SciPy's `find_peaks` function to determine nuclei.
    ...
    height : float, optional
        See `scipy.signal.find_peaks` documentation for explanation. Default None.
    threshold : float, optional
        See `scipy.signal.find_peaks` documentation for explanation. Default None.
    distance : float, optional
        See `scipy.signal.find_peaks` documentation for explanation. Default None.
    prominence : float, optional
        See `scipy.signal.find_peaks` documentation for explanation. Default 1.3.
    width : float, optional
        See `scipy.signal.find_peaks` documentation for explanation. Default None.
    rel_height : float, optional
        See `scipy.signal.find_peaks` documentation for explanation. Default 0.0.
    ...
    -------
    nuclei : array
        Array of nuclei start times.
    """

    peaks_idxs = find_peaks(
        intensity,
        height=height,
        threshold=threshold,
        distance=None,
    ...
    nuclei = ts[peaks_idxs]

    if verbose:
        print(f"Found {len(nuclei)} nuclei")
    ...
    **kwargs,
    ...
    for t1, t2 in zip(note_boundaries[:-1], note_boundaries[1:]):
        ...
        if len(p_note) == 0 or sum(np.isfinite(p_note)) / len(p_note) < 0.25:
            if verbose:
                print(" - note at {:.2f}: too unpitched, ignoring".format(t1))
            continue
        ...
        try:
            ...
            if verbose:
                print(" - note at {:.2f}: start not found, ignoring".format(t1))
            continue
        ...
        if t2 - start < minLength:
            ...
            if verbose:
                print(
                    " - note at {:.2f}: too short ({} < {}), ignoring".format(
                        t1, t2 - start, minLength
                    )
                )
            continue
        ...
        if method in ("derivatives", "max", "max_stable"):
            ...
            if p_stability > maxUnstable:
                ...
                if verbose:
                    print(
                        " - note at {:.2f}: unstable ({:.2f} > {:.2f})".format(
                            t1, p_stability, maxUnstable
                        )
                    )
                mid = len(p_note) // 2
                ...
                if verbose:
                    print(
                        " - note at {:.2f}: stable ({:.2f} <= {:.2f})".format(
                            t1, p_stability, maxUnstable
                        )
                    )
                regions = np.ma.clump_unmasked(
                    np.ma.masked_greater(np.abs(dp), unstable)
                )
                if verbose:
            ...
            raise (
                ValueError(
                    'method {} not in ("derivatives", "mean", "mean_stable", "max", "max_stable")'.format(
                        method
                    )
                )
            )
...
def retune_pitches(pitch, tuning=440):
...
def plot_notes(
    ...
    **kwargs,
    ...
    return_lines : bool, optional
        Return the line handle, useful for adding to a plot legend. Default False.
    **kwargs : optional
    ...
    ax : matplotlib.Axes
        Axes object.
    lines : matplotlib.Polygon
        Lines object. Only returned if `return_lines` is True.
    """
...
def midi_to_notes(fp, instrument=0, verbose=False):
    ...
    for n in ins.notes:
        ...
        if verbose:
            print(
                "  ({:.2f}, {:.2f}, {:.2f})".format(
                    n.start, n.end - n.start, midi_to_hz(n.pitch)
                )
            )
...
def create_audio(notes, end_time=None, save_path=None, sf=44100, octave=0, **kwargs):
    ...
    octave : float, optional
        How many octaves to shift the notes by, can be non-integer. Default 0.
    kwargs : optional
        Keyword arguments to pass onto tones.Track (see
        https://tones.readthedocs.io/en/latest/tones.html#tones.mixer.Track)
